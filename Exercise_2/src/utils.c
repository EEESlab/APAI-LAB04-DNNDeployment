...
void __attribute__ ((noinline)) dma_memcopy_3d(
  unsigned int ext,
  unsigned int loc,
  unsigned short stride_2d,
  unsigned short number_2d_copy,
  unsigned short length_2d_copy,
  unsigned int dir,
  unsigned int *id
) 
{
  if (pi_core_id()==0)
  {
    int offs_remote = 0;
    int offs_local = 0;
    for ( int i=0; i<number_2d_copy; i++) 
    {
      mchan_transfer(length_2d_copy, dir, 1, 0, 0, 1, 0, 0, (unsigned int)(ext + offs_remote), (unsigned int)(loc + offs_local), 0, 0, 0, 0);
      offs_local  += length_2d_copy;
      offs_remote += stride_2d;
    }
  }
}
